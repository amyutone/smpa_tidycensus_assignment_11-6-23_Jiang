...
census_api_key("0565d0e365cabc5e7c1f4eab3edce2107ff278ed", overwrite=TRUE)
```
...
```{r}
# Correct endpoint for fetching the variable list
variables_url <- "https://api.census.gov/data/2021/acs/acs5/variables.json"

# Use fromJSON to load the variables into a dataframe
variables_table <- fromJSON(variables_url)

# The variables are usually in a nested list within the dataframe, so we need to unnest it
variables_df <- as.data.frame(variables_table$variables)
